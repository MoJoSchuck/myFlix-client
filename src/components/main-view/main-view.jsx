...
export const MainView = () => {
    ...
    const handleUserUpdate = async (updatedUser) => {
        try {
            const response = await fetch(`https://afternoon-sands-47cb04422b71.herokuapp.com/users/${user.Username}`, {
                method: "PUT",
                headers: {
    ...
    const handleUserDelete = async (username) => {
        try {
            const response = await fetch(`https://afternoon-sands-47cb04422b71.herokuapp.com/users/${username}`, {
                method: "DELETE",
                headers: { Authorization: `Bearer ${token}` }
            });
            if (!response.ok) {
    ...
    const addFavorite = async (movie) => {
        try {
            const response = await fetch(`https://afternoon-sands-47cb04422b71.herokuapp.com/users/${user.Username}/movies/${movie._id}`, {
                method: "PATCH",
                headers: { Authorization: `Bearer ${token}` }
            });
            if (!response.ok) {
